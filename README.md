...
> ## ⚠️ Дисклеймер: используйте на свой страх и риск
>
> Проект обращается к API сервиса **1С:Напарник** (`code.1c.ai`) из стороннего клиента.
> По [пользовательскому соглашению](https://code.1c.ai/user_agreement/?lang=ru) API
> предназначено для работы из **1С:EDT**; вызов из внешних приложений соглашением
> не предусмотрен и трактуется сервисом как нарушение.
>
> Сам 1С:Напарник начал детектировать и напоминать об этом: если запрос пришёл не из 1С:EDT,
> модель получает указание в мягкой форме сообщить пользователю, что API предназначено
> для 1С:EDT. Поэтому в ответах периодически может появляться соответствующая приписка.
>
> Проект сделан в исследовательских целях. **Ответственность за использование — целиком
> на вас**, включая возможную блокировку токена или учётной записи. Автор не связан
> с фирмой «1С» и никаких гарантий не даёт.
>
> Если вы разработчик и у вас есть идеи сотрудничества — фирма «1С» предлагает писать
> на **ailab@1c.ru**.
